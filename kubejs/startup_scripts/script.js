...
onEvent('item.registry', event => {
	...
	event.create('cooked_worm').displayName('지렁이 구이').texture('kubejs:item/worm').food(food => {food.hunger(2);food.saturation(1);food.fastToEat(true);})
	event.create('cooked_apple').displayName('구운 사과').texture('kubejs:item/apple').food(food => {food.hunger(5);food.saturation(0.7);food.fastToEat(true);})
})
